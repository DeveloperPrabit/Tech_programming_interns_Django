...
from main.models import CustomUser,Category,Subcategory,News,Page,Comments
...
def Add_post(request):
    ...
    if request.method == "POST":
        ...
        # Validate that cat_id and subcategory_id are not empty or invalid
        if not cat_id or not subcategory_id_value:
            messages.error(request, 'Category or Subcategory is not selected.')
            return redirect("add_post")

        try:
            # Fetch category and subcategory objects based on their primary keys
            cid = Category.objects.get(id=cat_id)
            subcategory = Subcategory.objects.get(id=subcategory_id_value)
        ...
        # Ensure the user type is properly retrieved
        userregt = request.user.user_type if hasattr(request.user, 'user_type') else 'default_user_type'
...
def Subadmin_profile_update(request):
    ...
    user1 = get_object_or_404(CustomUser, id=request.user.id, user_type=2)
